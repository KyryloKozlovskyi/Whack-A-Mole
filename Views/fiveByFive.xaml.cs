...
public partial class fiveByFive : ContentPage
{
    //random
    ...
    public fiveByFive() //Initialize all
    {
        InitializeComponent();
        random = new Random();
        SetUpTimers();
    ...
    private void btnFiveByFiveStart_Clicked(object sender, EventArgs e)
    {
        MovetheMole();
        timer.Start();
        //disables start button
        btnFiveByFiveStart.IsEnabled = false;
        //enables reset button
        btnFiveByFiveReset.IsVisible = true;
        btnFiveByFiveReset.IsEnabled = true;
    }

    async private void btnFiveByFiveReset_Clicked(object sender, EventArgs e)
    {
        ...
        if (answer)
        {
            ...
            btnFiveByFiveReset.IsVisible = false;
            btnFiveByFiveReset.IsEnabled = false;
            btnFiveByFiveStart.IsEnabled = true;
        }
        else
        {
            //continues the game
    ...
    async private void TapGestureRecognizer_Tapped(object sender, EventArgs e) //taps
    {
        if (isTapEnabled)
        {
            if (countdown > 0 && btnFiveByFiveStart.IsEnabled == false)
            {
                isTapEnabled = false;
